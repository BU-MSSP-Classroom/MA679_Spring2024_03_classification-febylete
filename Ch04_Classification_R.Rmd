...
Your code:
```{r,echo=TRUE}
Train_COVID$SURVIVED_bin <-1*(Train_COVID$SURVIVED=="yes")
Train_COVID$PNEUMONIA <- ifelse(is.na(Train_COVID$PNEUMONIA), "yes", Train_COVID$PNEUMONIA)


covid_model1 <- glm(SURVIVED_bin ~ AGE_TRUNK + FEMALE, data = Train_COVID, family = binomial)
covid_model2 <- glm(SURVIVED_bin ~ AGE_TRUNK + FEMALE + PNEUMONIA + HIPERTENSION + CARDIOVASCULAR , data = Train_COVID, family = binomial)
...
~~~
I tried to fit 2 model so I can answer the second question. For first model I used the demographics characteristics which are age and sex. The second model I added the comorbidities factors. 
...
predictions1 <- predict(covid_model1, data = Train_COVID, type = "response")
logit_pred1 <- ifelse(predictions1 > 0.5,1,0)
ConMat1 = table(Train_COVID$SURVIVED_bin, logit_pred1)
print(ConMat1)

predictions2 <- predict(covid_model2, data = Train_COVID, type = "response")
logit_pred2 <- ifelse(predictions2 > 0.5,1,0)
ConMat2 = table(Train_COVID$SURVIVED_bin, logit_pred2)
print(ConMat2)
...
~~~
From the confusion matrix above, my second model seems better than my first model. Because the false positive and false negative is smaller than the first model.
...
Auto <- data.frame(Auto %>% mutate(mpg01 = ifelse(mpg > median(mpg), 1, 0)))
```
...
ggplot(Auto, aes(horsepower, mpg)) +
  geom_point(aes()) +
  labs(title = "Scatter Plot of mpg by horsepower",
       x = "Horsepower",
       y = "Miles Per Gallon")

ggplot(Auto, aes(acceleration, mpg)) +
  geom_point(aes(color = cylinders)) +
  labs(title = "Scatter Plot of mpg by acceleration",
       x = "Acceleration",
       y = "Miles Per Gallon")
```
